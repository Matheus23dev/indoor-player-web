...
import type { Device, DeviceLog, PairDevicePayload } from "../types/device";
...
export async function pairDevice(code: string, name: string): Promise<Device> {
  ...
  const response = await instance.post<Device>("/devices/pair", payload);

  return response.data;
}

export async function getDeviceLogs(deviceId: string): Promise<DeviceLog[]> {
  const response = await instance.get<DeviceLog[]>(`/devices/${deviceId}/logs`);

  return response.data;
}

export async function unlinkDevice(deviceId: string) {
  const response = await instance.post(`/devices/${deviceId}/unlink`);
