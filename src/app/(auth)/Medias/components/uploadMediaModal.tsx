...
export default function UploadMediaModal({
  ...
  function isDuplicate(newFile: File, currentFiles: File[]) {
  ...
  function validateFiles(selectedFiles: File[]) {
    if (files.length + selectedFiles.length > MAX_FILES) {
      void Swal.fire({
        ...
        text: "É permitido selecionar no máximo 20 mídias por upload.",
      });
  ...
  function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const newFiles = selectedFiles.filter((selectedFile) => !isDuplicate(selectedFile, files));

    if (newFiles.length === 0) {
      void Swal.fire({
        icon: "info",
        title: "Arquivos já selecionados",
        text: "Os arquivos escolhidos já estão na lista.",
      });
  ...
  async function handleUpload() {
    if (files.length === 0) {
      await Swal.fire({
        ...
        text: "Escolha pelo menos um arquivo.",
      });
    ...
    try {
      ...
      for (let index = 0; index < files.length; index += 1) {
        ...
        await uploadMedia(files[index], folderId || null);
      ...
      const message = getApiErrorMessage(error, "Não foi possível enviar as mídias.");
      ...
      await Swal.fire({
        ...
        text:
          uploadedCount > 0
            ? `${uploadedCount} mídia(s) foram enviadas antes do erro. ${message}`
            : message,
      });
    } finally {
      setUploading(false);
  ...
  const progress = files.length > 0 ? Math.min(100, (currentUpload / files.length) * 100) : 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6">
      <div className="flex max-h-[95vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <header className="flex items-center justify-between border-b px-6 py-4">
          <div>
            <h2 className="text-xl font-black text-gray-900">Enviar mídias</h2>

            <p className="text-sm text-gray-500">Selecione até 20 arquivos por envio.</p>
          </div>
        ...
        <div className="space-y-5 overflow-y-auto p-6">
          <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-black text-blue-950">Recomendações para envio de arquivos</h3>

                <div className="mt-2 space-y-1 text-sm text-blue-900">
                  <p>
                    <strong>Tipos aceitos:</strong> PNG, JPG, JPEG e MP4.
                  </p>

                  <p>
                    <strong>Dimensões recomendadas:</strong> 1920×1080, 1080×1920, 1280×720 ou
                    720×1280.
                  </p>

                  <p>
                    <strong>Tamanho máximo:</strong> 80 MB por arquivo.
                  </p>

                  <p>
                    <strong>Limite:</strong> no máximo 20 mídias por upload.
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <p className="font-black text-gray-900">Clique para selecionar arquivos</p>

            <p className="mt-1 text-sm text-gray-500">PNG, JPG, JPEG e MP4</p>

            <p className="mt-1 text-xs font-bold text-gray-400">{files.length}/20 selecionados</p>
          ...
          {files.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-4">
                <h3 className="font-black text-gray-900">Arquivos selecionados</h3>
            ...
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-bold text-blue-900">
                  Enviando mídia {currentUpload} de {files.length}
                </p>

                <span className="text-xs font-black text-blue-700">{Math.round(progress)}%</span>
              </div>
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-3">
      ...
      <div className="min-w-0 flex-1">
        <p title={file.name} className="truncate text-sm font-bold text-gray-900">
          {file.name}
        </p>
